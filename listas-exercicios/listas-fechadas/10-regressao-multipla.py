...
def descent(theta0, X, y, learning_rate=0.005, tolerance=1e-7):
    theta = theta0.copy()
    ...
    while i < 10000:
      ...
      erro_novo = ((X.dot(theta) - y) ** 2).mean()

      if abs(erro_anterior - erro_novo) <= tolerance:
        break

      theta = novo_theta
      erro_anterior = erro_novo
      i += 1
...
def sst(y):
  soma_total_quadrados = ((y - y.mean()) ** 2).sum()
  return soma_total_quadrados

def loss_um_ponto(X, y, theta):
    return (y - X @ theta) ** 2

def sse(X, y, theta):
    return loss_um_ponto(X, y, theta).sum()

def rsquared(X, y, theta):
  r_quadrado = 1 - sse(X, y, theta) / sst(y)
  return r_quadrado
